...
def process_hub5(method = 1, joint = True, data = None):
	...
	for gap_length in gap_length_arr:
		tmpT = []
		tmpF = []
		tmpT1 = []
		tmpF1 = []
		for times in range(20):
			...
			print(patch_arr)
			print("frame missing: ", gap_length, "times: ",times)
			...
			for x in range(number_patch):
				if patch_arr[x] > 0:
					...
					missing_matrix = generate_missing_joint(
						sample.shape[0], sample.shape[1], gap_length)		
