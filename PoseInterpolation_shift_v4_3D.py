...
def process_hub5(method = 1, joint = True):
	...
	A_temp_zero = []
	l = [x for x in range(arg.length3D)]
	missing_frame_arr = random.sample(l, 15)
	for num_missing in arg.missing_number:
		if joint:
			A_temp_zero.append(get_random_joint3D(A, arg.length3D, num_missing, missing_frame_arr))
	...
	for current_frame_shift in range(4):
		...
		for index_A_temp in range(len(arg.missing_number)):
			...
			A1_star3 = interpolation_13_v3(np.copy(A_N3), np.copy(A) ,np.copy(A1zero),
							shift = check_shift, option = None, missing_number = index_A_temp+1)
			...
			A1_star3o, _,_,_ = interpolation_13_v2(np.copy(A_N3), np.copy(A) ,np.copy(A1zero),
							shift = check_shift, option = None, missing_number = index_A_temp+1)
			tmpA3_old.append(np.around(calculate_mse(A1, A1_star3o), decimals = 17))

			A1_star4o, _,_,_,_ = interpolation_24_v2(np.copy(A_N), np.copy(A) ,np.copy(A1zero),
							shift = check_shift, option = None, missing_number = index_A_temp+1)
