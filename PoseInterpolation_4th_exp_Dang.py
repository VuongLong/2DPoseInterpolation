...
def generate_missing_joint(n, m, frame_length, number_gap, starting_frame):
	...
	while counter < number_joint:
		...
		while tmp in joint_in:
			tmp = random.randint(0, m//3-3)
		start_missing_frame = random.randint(0, n - frames)
		missing_joint = tmp
		...
		for times in range(1):
			...
			for x in range(number_patch):
				...
				if patch_arr[x] > 0:
					...
					missing_matrix = generate_missing_joint(
						sample.shape[0], sample.shape[1], lmiss, 1*patch_arr[x], starting_frame_A1)		
			...
			for x in range(number_patch):
				if patch_arr[x] > 0:
					...
					A1_star3 = interpolation_13_v7(np.copy(A_N3),np.copy(A1zero))
					...
					# compute 2nd method
					A1_star4 = interpolation_24_v4(np.copy(A_N),np.copy(A1zero), Tracking3D)
