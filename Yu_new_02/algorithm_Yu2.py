...
class Interpolation16th_F():
	...
	# the below function is similar compute svd in PCA
	def compute_svd(self):
		...
		if add_small_patch:
			...
			self.weight_sample = [0.8/self.K]*self.K
			self.weight_sample[-1] += 0.2


	# the below function compute svd in our own way
	def compute_svd_origin(self):
		M_zero = self.normed_matries[0]
		r = self.fix_leng
		l = 0
		add_small_patch = False
		ksmall = 0
		while r <= M_zero.shape[0]:
			self.K += 1
			tmp = np.copy(M_zero[l:r])
			self.list_A.append(np.copy(tmp))
